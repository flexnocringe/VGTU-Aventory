  ...
  return (
    <section className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
      <div className="flex items-start justify-between">
        <div>
          <h2 className="text-xl font-semibold text-slate-900">Stock distribution</h2>
          <p className="mt-1 text-sm text-slate-600">Top products by quantity currently in inventory.</p>
        </div>
        <span className="rounded-full bg-slate-100 px-3 py-1 text-xs font-medium text-slate-700">Live</span>
      </div>

      <div className="mt-6">
        {loading ? (
          <p className="text-sm text-slate-500">Calculating chart values...</p>
        ) : barItems.length === 0 ? (
          <p className="text-sm text-slate-500">No products available for charting.</p>
        ) : (
